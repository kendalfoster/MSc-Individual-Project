...
transformed data { // manipulations of the data to use in parameter bounds
  int upperH = max(NTI) - 5; // (rough) upper bound for H
}
...
parameters { // define parameters (and their bounds) used in the model
  ...
  vector<lower=0.1, upper=1>[K] D;  // scale of learning "drop"
  ...
  vector<lower=0, upper=upperH>[K] H;  // horizontal shift of onset of learning
  vector<lower=0, upper=upperH>[K] h_loc;
  vector<lower=0>[K] h_scale;

  vector<lower=0>[K] sigma;  // standard deviation in response times
}

model { // define the Bayesian hierarchical model
  // Initialize this outside the loops
  real mu0;
  real mu1;
  int N;
  int st = 0;
  ...
  for (i in 1:K) { // iterate through each individual
    ...
    for (trial in 1:N) { // cannot vectorize because cannot divide by vector
      ...
      mu1 = log( ( V[i] + E[i]*exp(-A[i]*trial) ) * (1 - D[i]/(1 + exp(-L[i]*(trial-H[i]))) ) );
