...
plot_fits_gg <- function(k, NTI, y0, y1, fits, save=FALSE) {
  ...
  for (i in seq_len(k)) {
    ...
    df <- data.frame(x = c(seq_len(n), seq_len(n)),
                     ...
                     fit = c(exp_log(x[1:n], V=fits[[i]], E=fits[[i+3*k]], A=fits[[i+4*k]], D=0, L=fits[[i+9*k]], H=fits[[i+12*k]]),
                             exp_log(x[1:n], V=fits[[i]], E=fits[[i+3*k]], A=fits[[i+4*k]], D=fits[[i+5*k]]*fits[[i+6*k]], L=fits[[i+9*k]], H=fits[[i+12*k]])),
    ...
    if (save) {
      ggsave(paste0("Bayesian-Hierarchical-Model/Images/fits_gg_", i, ".png"))
    }
    st = st + n
  }
}
...
plot_fits_sim_gg <- function(k, NTI, V, E, A, D, L, H, y0, y1, fits, save=FALSE) {
  ...
  for (i in seq_len(k)) {
    ...
    df <- data.frame(x = c(seq_len(n), seq_len(n)),
                     ...
                     fit = c(exp_log(x, V=fits[[i]], E=fits[[i+3*k]], A=fits[[i+4*k]], D=0, L=fits[[i+9*k]], H=fits[[i+12*k]]),
                             exp_log(x, V=fits[[i]], E=fits[[i+3*k]], A=fits[[i+4*k]], D=fits[[i+5*k]]*fits[[i+6*k]], L=fits[[i+9*k]], H=fits[[i+12*k]])),
                     fit_min = c(exp_log(x, V=fits[[i]], E=fits[[i+3*k]], A=fits[[i+4*k]], D=0, L=fits[[i+9*k]], H=fits[[i+12*k]]),
                                 exp_log(x, V=fits[[i]], E=fits[[i+3*k]], A=fits[[i+4*k]], D=fits[[i+5*k]]*fits[[i+6*k]], L=fits[[i+9*k]], H=fits[[i+12*k]]))
                                 - fits[[i+15*k]],
                     fit_max = c(exp_log(x, V=fits[[i]], E=fits[[i+3*k]], A=fits[[i+4*k]], D=0, L=fits[[i+9*k]], H=fits[[i+12*k]]),
                                 exp_log(x, V=fits[[i]], E=fits[[i+3*k]], A=fits[[i+4*k]], D=fits[[i+5*k]]*fits[[i+6*k]], L=fits[[i+9*k]], H=fits[[i+12*k]]))
                                 + fits[[i+15*k]],
                     label_data = c(rep("Non-Learned Data",n), rep("Learned Data",n)),
                     label_fit = c(rep("Non-Learned Fit",n), rep("Learned Fit",n)))
    ...
    if (save) {
      ggsave(paste0("Bayesian-Hierarchical-Model/Images/sim_gg_", i, ".png"))
    }
    st = st + n
  }
}
